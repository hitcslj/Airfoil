...
def parse_option():
    ...
    # io
    parser.add_argument('--checkpoint_path', default='eval_result/logs_cvae/ckpt_epoch_10000.pth',help='Model checkpoint path')
    parser.add_argument('--log_dir', default='./test_result/logs_cvae',
...
class Tester:
    ...
    def evaluate_one_epoch(self, model,criterion, dataloader,device, epoch, args):
        ...
        for _,data in enumerate(test_loader):
            ...
            recon_batch, mu, logvar = model.forward3(gt,condition) # [b,257,2],[b,37,2]

            recon_batch = recon_batch.reshape(-1,257,2)

            total_pred += keypoint.shape[0]
            # loss,_ = criterion(data,output)
            loss = criterion(gt,recon_batch)
            total_loss += loss.item()
            # 判断样本是否预测正确
            distances = torch.norm(gt - recon_batch,dim=2) #(B,200)
    ...
    def test(self,args):
        ...
        test_dataset = AirFoilDatasetParsec(split='test')
        ...
        for step, data in enumerate(test_loader):
            ...
            with torch.no_grad():
                ...
                output = recon_batch.reshape(-1,257,2)
                data = gt
                # 计算 recon_batch 和 gt 物理量的差距
                f_pred = Fit_airfoil(output[0].cpu().numpy()).parsec_features
                f_gt = Fit_airfoil(gt[0].cpu().numpy()).parsec_features
                for j in range(11):
                    cnt[parsec_features[j]] += abs(f_pred[j] - f_gt[j])
                # print(f'absrel error: {abs(f_pred-f_gt)/f_gt},     abs error:{abs(f_pred-f_gt)}')
                ## 将其写进日志
                # with open('log.txt', 'a') as file:
                #     file.write(f'absrel error: {abs(f_pred - f_gt) / f_gt}, abs error: {abs(f_pred - f_gt)}\n')
                ...
                sample_x = keypoint[0,:,0].cpu().numpy()
                sample_y = keypoint[0,:,1].cpu().numpy()
                ...
                plt.savefig(f'{args.log_dir}/{step}_recons.png', format='png')
                plt.close()
        # cnt统计的物理量误差写入日志
        with open('recons_parsec_features_error.txt','w') as f:
            for k,v in cnt.items():
                f.write(f'{k}:{v/len(test_loader)}\n')
