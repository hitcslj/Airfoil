...
from models import EP_VAE,PK_VAE,EP_PKVAE
...
def parse_option():
    ...
    # io
    parser.add_argument('--checkpoint_path_A', default='logs/ep_vae/ckpt_epoch_1000.pth',help='Model checkpoint path')

    parser.add_argument('--checkpoint_path_B', default='logs/pk_vae/ckpt_epoch_2000.pth',help='Model checkpoint path')

    parser.add_argument('--log_dir', default='logs/ep_pkvae',
...
class Trainer:
    ...
    def infer_param(self,args, model, dataloader,device, epoch):
        ...
        for _,data in enumerate(test_loader):
            ...
            target_param = target_param.to(device)
            gt = gt.to(device)
            _,recon_batch = model(source_param, target_param, source_keypoint) 
