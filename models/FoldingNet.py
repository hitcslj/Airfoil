...
import numpy as np
from models.utils import index_points, knn
...
class Encoder(nn.Module):
    ...
    def forward(self, x):
        ...
        x = torch.cat([x, covariances], dim=1)  # (B, 6, N)
...
class Decoder(nn.Module):
    ...
    def __init__(self, in_channel=512):
        ...
        xx = np.linspace(-0.3, 0.3, 10, dtype=np.float32)
        yy = np.linspace(-0.3, 0.3, 20, dtype=np.float32)
        self.grid = np.array(np.meshgrid(xx, yy))   # (2, 45, 45)
        ...
        self.fold1 = FoldingLayer(in_channel + 2, [512, 512, 2])
        self.fold2 = FoldingLayer(in_channel + 2, [512, 512, 2])
...
if __name__ == '__main__':
    pcs = torch.randn(32, 2, 200) # (B, 2, N)

    encoder = Encoder()
    codewords = encoder(pcs)
    print(codewords.size()) # (B, 512)

    decoder = Decoder(codewords.size(1))
    recons = decoder(codewords)
    print(recons.size()) # (B, 2, 200)

    ae = FoldingNet()
    y = ae(pcs) 
    print(y.size()) # (B, 2, 200)